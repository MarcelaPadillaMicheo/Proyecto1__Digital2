...
void I2C_Nack(void)
{
    SSPCON2bits.ACKDT = 1;  // Configura el bit ACKDT para generar NACK
    SSPCON2bits.ACKEN = 1;  // Activa la generación de NACK
    while(PIR1bits.SSPIF == 0);  // Espera a que la operación I2C se complete
    PIR1bits.SSPIF = 0;  // Reinicia el bit de interrupción I2C
}

void I2C_Ack(void)
{
    SSPCON2bits.ACKDT = 0;  // Configura el bit ACKDT para generar ACK
    SSPCON2bits.ACKEN = 1;  // Activa la generación de ACK
    while(PIR1bits.SSPIF == 0);  // Espera a que la operación I2C se complete
    PIR1bits.SSPIF = 0;  // Reinicia el bit de interrupción I2C
}
